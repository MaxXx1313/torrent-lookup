...
class Analyzer extends EventEmitter {
  ...
  analyze(){
    return this.loadTorrentFiles(this._options.tFile)
      // .then(this.matchFiles.bind(this))
      // .then(this.regroup.bind(this))
      // .then(this._makeDecision.bind(this))
      // .then(this._saveJson.bind(this, this._options.rFile))
  }


  loadTorrentFiles(torrentFile){
    var self = this;
    self.emit('status', 'Hashing data');
  ...
  saveTo(location){
    ...
    return new Promise((resolve, reject)=>{
      fs.writeFile(location, JSON.stringify(self._decision), function(err, result){
        if(err){
          reject(err)
        } else {
          resolve(result);
