...
class Analyzer extends EventEmitter {
  ...
  constructor(options){
    ...
    // assert.ok(options.dFile);
    // assert.ok(options.tFile);
    // assert.ok(options.rFile);

    this._options = {};
    this._options.dFile = options.data  || '/tmp/files.bin';
    this._options.tFile = options.tdata || '/tmp/torrents.bin';
    this._options.rFile = options.rdata || '/tmp/maps.json';
  ...
  saveTo(location){
    ...
    return writeFile(location, JSON.stringify(self._decision));
  }
...
function writeFile(location, data, opts){
  return new Promise((resolve, reject)=>{
    fs.writeFile(location, data, opts, function(err, result){
      if(err){
        reject(err)
      } else {
        resolve(result);
