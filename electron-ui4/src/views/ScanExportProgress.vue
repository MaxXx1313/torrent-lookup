<template>
  ...
  <main class="flex-1 flex flex-col overflow-y-auto">
    ...
    <div class="max-w-4xl mx-auto w-full px-8 py-8">
      <!-- Progress State -->
      <div class="mb-10 text-center relative" v-if="!exportIsFinished">
        <div
        ...
        <h1 class="text-slate-900 dark:text-white text-3xl font-bold leading-tight mb-2">
          Exporting to Transmission...
        </h1>
        <p class="text-slate-500 dark:text-[#92adc9]">Moving matched metadata to your local torrent client instance</p>
      </div>

      <!-- Progress Bar Component -->
      <div v-if="!exportIsFinished"
           class="bg-white dark:bg-[#1a2632] rounded-xl p-8 border border-slate-200 dark:border-[#233648] shadow-sm mb-8">
        <div class="flex flex-col gap-4">
          <div class="flex justify-between items-end">
            <div class="flex flex-col gap-1">
              <p class="text-slate-900 dark:text-white text-lg font-bold" v-if="stats?.total">
                {{ stats.completed }} of {{ stats.total }} items exported
              </p>
              <p class="text-slate-900 dark:text-white text-lg font-bold" v-if="!stats?.total">
                Initializing...
              </p>
              ...
              <p class="text-slate-500 dark:text-[#92adc9] text-sm font-normal">
                Current: <span class="text-primary">{{ stats?.currentTarget || '...' }}</span>
              </p>
            </div>
            <p class="text-primary text-2xl font-bold leading-none">{{ stats?.percentage || 0 }}%</p>
          </div>
          <div class="h-3 w-full rounded-full bg-slate-100 dark:bg-[#324d67] overflow-hidden">
            <div
                ...
                :style="`width: ${stats?.percentage || 0}%`"></div>
          </div>
        </div>
      </div>
      ...
      <main v-if="exportIsFinished && hasError">
        <div class="max-w-4xl mx-auto w-full px-8 py-8">
          <div class="bg-amber-500/10 border border-amber-500/20 rounded-xl p-8 flex flex-col items-center text-center">
            <div
                class="w-16 h-16 text-xl rounded-full bg-amber-500 flex items-center justify-center text-white mb-4 shadow-lg shadow-amber-500/20">
...
const exportIsFinished = ref<boolean>(false);
